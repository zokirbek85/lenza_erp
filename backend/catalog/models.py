...
class DefectType(models.Model):
    """
    Reference-справочник типов дефектов.
    Управляется администратором.
    Примеры: "дошечка", "оына", "ён стойевой", "қоплама", и т.д.
    """
    name = models.CharField(
        max_length=200,
        unique=True,
        verbose_name="Название типа дефекта",
        help_text="Тип дефекта (например: дошечка, стекло, стойка)"
    )
    description = models.TextField(
        blank=True,
        verbose_name="Описание",
        help_text="Дополнительное описание типа дефекта"
    )
    is_active = models.BooleanField(
        default=True,
        verbose_name="Активен",
        help_text="Активен ли этот тип дефекта"
    )
...
class ProductDefect(models.Model):
    """
    Дефект продукта.
    Агрегированная карточка по продукту (не по каждому отдельному дефектному item).
    
    Пример:
    - product: "Eshik Бета Софт 800мм"
    - qty: 3
    - repairable_qty: 1
    ...
    class Status(models.TextChoices):
        ...
        REPAIRED = 'repaired', 'Отремонтирован'
        DISPOSED = 'disposed', 'Утилизирован'
        SOLD_OUTLET = 'sold_outlet', 'Продан outlet'
    ...
    product = models.ForeignKey(
        ...
        help_text="Продукт с дефектом"
    )
    qty = models.DecimalField(
        max_digits=14,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name="Общее количество дефектов",
        help_text="Общее количество дефектных единиц"
    )
    repairable_qty = models.DecimalField(
        max_digits=14,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name="Количество ремонтопригодных",
        help_text="Количество единиц, которые можно отремонтировать"
    )
    non_repairable_qty = models.DecimalField(
        max_digits=14,
        decimal_places=2,
        default=Decimal('0.00'),
        verbose_name="Количество неремонтопригодных",
        help_text="Количество единиц, которые нельзя отремонтировать"
    )
    defect_details = models.JSONField(
        default=list,
        blank=True,
        verbose_name="Детали дефектов",
        help_text='JSON массив: [{"type_id": 1, "type_name": "дошечка", "qty": 2}]'
    )
    ...
    description = models.TextField(
        blank=True,
        verbose_name="Описание",
        help_text="Дополнительное описание дефекта"
    )
    repair_materials = models.JSONField(
        default=list,
        blank=True,
        verbose_name="Материалы для ремонта",
        help_text='JSON массив: [{"product_id": 123, "product_name": "...", "qty": 2}]'
    )
    repair_completed_at = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Дата завершения ремонта",
        help_text="Когда был завершен ремонт"
    )
    disposed_at = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Дата утилизации",
        help_text="Когда был утилизирован"
    )
    sold_outlet_at = models.DateTimeField(
        null=True,
        blank=True,
        verbose_name="Дата продажи outlet",
        help_text="Когда был продан через outlet"
    )
    created_by = models.ForeignKey(
        ...
        verbose_name="Создал",
        help_text="Пользователь, создавший запись о дефекте"
    )
    updated_by = models.ForeignKey(
        'users.User',
        on_delete=models.SET_NULL,
        null=True,
        related_name='updated_defects',
        verbose_name="Обновил",
        help_text="Пользователь, последним обновивший запись"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Дата обновления")
    ...
    def __str__(self) -> str:
        return f"Defect #{self.id}: {self.product.name} ({self.qty} шт)"
    
    def clean(self):
        """Validate that qty = repairable_qty + non_repairable_qty"""
        from django.core.exceptions import ValidationError
        
        total = self.repairable_qty + self.non_repairable_qty
        if abs(total - self.qty) > Decimal('0.01'):
            raise ValidationError({
                'qty': f'Общее количество ({self.qty}) должно равняться сумме ремонтопригодных ({self.repairable_qty}) и неремонтопригодных ({self.non_repairable_qty})'
            })
...
class DefectAuditLog(models.Model):
    """
    Audit log для отслеживания всех изменений дефектов.
    Каждое изменение статуса, количества, ремонта записывается сюда.
    """
    ...
    old_data = models.JSONField(
        ...
        help_text="Снимок данных до изменения"
    )
    new_data = models.JSONField(
        null=True,
        blank=True,
        verbose_name="Новые данные",
        help_text="Снимок данных после изменения"
    )
    description = models.TextField(
        blank=True,
        verbose_name="Описание",
        help_text="Дополнительное описание действия"
    )
    user = models.ForeignKey(
        'users.User',
        on_delete=models.SET_NULL,
        null=True,
        related_name='defect_audit_logs',
    ...
    def __str__(self) -> str:
        return f"{self.get_action_display()} - Defect #{self.defect.id} by {self.user}"

...
export default function PaymentTrendChart({ data, loading }: PaymentTrendChartProps) {
  ...
  console.log('PaymentTrendChart - Data:', data);
  console.log('PaymentTrendChart - Loading:', loading);
  console.log('PaymentTrendChart - Data length:', data?.length);
  ...
  // Agar ma'lumot bo'sh bo'lsa, test data ko'rsatamiz
  const chartData = data && data.length > 0 ? data : [
    { date: '2025-01-01', amount: 1200 },
    { date: '2025-01-02', amount: 1500 },
    { date: '2025-01-03', amount: 2200 },
  ];
  ...
  return (
    <Card className="dashboard-card payment-chart-card" bordered={false}>
      <div className="card-header">
        ...
        <h2 className="dashboard-card-title">{t('dashboard.paymentTrend', "To'lovlar dinamikasi")}</h2>
      </div>

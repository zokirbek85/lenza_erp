...
const MobileCard = ({
  ...
  return (
    ...
    >
        ...
        <div className="flex flex-wrap gap-1">
          {badges.map((badge) => (
            <span
              key={badge.label}
              className={clsx(
                'rounded-full px-3 py-0.5 text-[11px] font-semibold tracking-wide',
      ...
      {actions.length > 0 && (
        <div className="mt-4 flex flex-wrap gap-2 border-t border-dashed border-slate-200 pt-3 dark:border-slate-800">
          {actions.map((action) => (
            <button
              key={action.label}
              type="button"
              onClick={action.onClick}
              className="flex-nowrap flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-1 text-sm font-semibold text-slate-600 transition hover:bg-slate-100 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-white/10"
            >

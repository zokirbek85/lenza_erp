...
# Check if .env exists
if [ ! -f ".env" ]; then
    log_error ".env file not found. Run server_install.sh first."
    exit 1
fi
...
docker compose -f "deploy/docker-compose.${INITIAL_STACK}.yml" build --no-cache

log_step "3. Starting Database and Redis"

# Start shared services first
log_info "Starting database and Redis..."
docker compose -f "deploy/docker-compose.${INITIAL_STACK}.yml" up -d db redis
...
max_attempts=30
attempt=0
while ! docker compose -f "deploy/docker-compose.${INITIAL_STACK}.yml" exec -T db pg_isready -U "${POSTGRES_USER}" > /dev/null 2>&1; do
    attempt=$((attempt + 1))
...
docker compose -f "deploy/docker-compose.${INITIAL_STACK}.yml" up -d backend_${INITIAL_STACK}

# Wait for backend to start
sleep 5

# Migrations are run automatically by entrypoint script
log_info "Migrations completed automatically by entrypoint"
...
echo
if [[ $REPLY =~ ^[Yy]$ ]]; then
    docker compose -f "deploy/docker-compose.${INITIAL_STACK}.yml" exec backend_${INITIAL_STACK} python manage.py createsuperuser
fi

log_step "6. Starting Frontend"

log_info "Starting frontend..."
docker compose -f "deploy/docker-compose.${INITIAL_STACK}.yml" up -d frontend_${INITIAL_STACK}
...
# Ensure active_upstream.conf points to blue
